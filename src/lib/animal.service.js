...
export const animalService = {
  list(ownerId) { return read().filter(a => a.owner_id === ownerId).map(normalize); },
  get(id) { const a = read().find(x => x.id === id); return a ? normalize(a) : null; },
  create(ownerId, dto) {
    const data = read();
  ...
  isEarringUnique(ownerId, earring, ignoreId) {
    return !read().some(a => a.owner_id === ownerId && a.earring === earring && a.id !== ignoreId);
  }
};
