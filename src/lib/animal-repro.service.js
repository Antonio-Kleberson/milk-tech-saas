...
export const animalRepro = {
  ...
  listByAnimal(animal_id) {
    return this.list().filter((x) => x.animal_id === animal_id);
  },
  create(dto) {
  ...
  lastService(animal_id) {
    const events = this.listByAnimal(animal_id)
      .filter((e) => e.kind === "ia" || e.kind === "cobertura")
      .sort((a, b) => b.date.localeCompare(a.date));
    return events[0] || null;
  },
  lastDiagnosis(animal_id) {
    const events = this.listByAnimal(animal_id)
      .filter((e) => e.kind === "diagnostico")
      .sort((a, b) => b.date.localeCompare(a.date));
    return events[0] || null;
  },
