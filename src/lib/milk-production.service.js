...
export const milkProduction = {
  ...
  createOrUpdate(ownerId, dto) {
    ...
    const idx = data.findIndex(x =>
      x.owner_id === ownerId && sameDay(x.date, dto.date) && x.shift === dto.shift
    );
  ...
  update(id, dto) {
    ...
    const next = {
      ...
      ...(dto.unit_price_at_sale != null ? { unit_price_at_sale: Number(dto.unit_price_at_sale) } : {}),
    ...
    const conflictIdx = data.findIndex(
      (x, i) => i !== idx && x.owner_id === next.owner_id && sameDay(x.date, next.date) && x.shift === next.shift
    );
    if (conflictIdx >= 0) {
  ...
  sumMonth(ownerId, year, month) {
    ...
    return this.list(ownerId, { from, to })
  ...
  sumMonthByShift(ownerId, year, month) {
    ...
    const morning   = list.filter(x => x.shift === 'morning')
                          .reduce((s, x) => s + Number(x.liters || 0), 0);
    const afternoon = list.filter(x => x.shift === 'afternoon')
                          .reduce((s, x) => s + Number(x.liters || 0), 0);
  ...
  aggregateByDay(ownerId, { from, to } = {}) {
    ...
    this.list(ownerId, { from, to }).forEach(x => {
      ...
      const key = typeof x.date === 'string'
        ? x.date
