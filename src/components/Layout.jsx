...
import {
  Home, DollarSign, Truck, BookOpen, Calculator, LogOut, Milk, Droplets, MoreHorizontal, Settings
} from 'lucide-react';
...
const Layout = () => {
  ...
  const ProfileMenu = () => (
    <ul className="absolute mt-2 w-64 rounded-xl border bg-white/90 backdrop-blur shadow-lg p-1 right-0" role="menu">
      ...
      <nav className="bg-white/80 backdrop-blur border-b hidden md:block">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex h-16 items-center justify-between">
            ...
            <div className="relative">
              <button
                onClick={() => { setOpenProfile(v => !v); setOpenMore(false); }}
              ...
              >
                ...
                <span className="text-sm font-medium text-gray-800 max-w-[180px] truncate">{user?.name}</span>
              </button>
        ...
        <div className="grid grid-cols-5 h-full">
          {mobileItems.map((item, idx) => {
            ...
            return (
              ...
              >
                <motion.div whileTap={{ scale: 0.9 }} className={`rounded-xl p-1 ${active ? 'bg-green-50' : ''}`}>
        ...
        {openMore && (
          <div className="absolute bottom-14 left-0 right-0 px-3 pb-3 z-50">
            <div className="rounded-2xl border bg-white/95 backdrop-blur shadow-xl p-2">
              ...
              {user?.role === 'producer' && (
                <button
                  onClick={() => { navigate('/laticinios'); setOpenMore(false); }}
              ...
              {moreItems.map(mi => {
                ...
                return (
                  <button
                    key={mi.path}
                    onClick={() => { navigate(mi.path); setOpenMore(false); }}
                    className="w-full flex items-center gap-3 px-3 py-3 rounded-xl hover:bg-green-50 text-sm"
                  >
                    <Icon className="h-5 w-5" />
                    <span>{mi.label}</span>
              ...
              <button
                onClick={() => { navigate('/settings'); setOpenMore(false); }}
                className="w-full flex items-center gap-3 px-3 py-3 rounded-xl hover:bg-green-50 text-sm"
              >
