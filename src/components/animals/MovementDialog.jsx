...
export function MovementDialog({ open, onClose, onSubmit, initialData, animal }) {
  ...
  async function handleSubmit(ev) {
    ...
    await onSubmit?.({
      ...
      type, date, amount: amount === "" ? null : Number(amount), notes
    });
    onClose?.();
  }
  ...
  return (
    <Dialog open={open} onOpenChange={(v)=>!v && onClose?.()}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{isEdit ? `Editar movimentação - ${animal?.name ?? ""}` : `Nova movimentação - ${animal?.name ?? ""}`}</DialogTitle>
          <DialogDescription className="sr-only">Registrar compra, venda, óbito ou transferência do animal.</DialogDescription>
        </DialogHeader>
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          {(type === "compra" || type === "venda") && (
            <div className="grid gap-2">
              <Label>Valor (R$)</Label>
              <Input type="number" step="0.01" min="0" value={amount} onChange={(e)=>setAmount(e.target.value)} placeholder="Ex.: 3500.00" />
