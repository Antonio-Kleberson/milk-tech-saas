...
const Production = () => {
  ...
  const chartData = useMemo(() => {
    const base = dayFilter
      ? daysAgg.filter(d => d.date.slice(0,10) === dayFilter)
      : daysAgg;
  ...
  const exportCSV = () => {
    ...
    a.href = url; a.download = `producao_${month}.csv`; a.click();
  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content="Registre a produção diária (manhã/tarde), visualize gráficos e exporte seus dados." />
      </Helmet>
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600">Registre sua produção de <strong>Manhã</strong> e <strong>Tarde</strong> sem duplicar o dia.</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <Card><CardContent className="p-4"><p className="text-sm text-gray-600">Manhã (mês)</p><p className="text-2xl font-bold">{monthTotals.manha} L</p></CardContent></Card>
          <Card><CardContent className="p-4"><p className="text-sm text-gray-600">Tarde (mês)</p><p className="text-2xl font-bold">{monthTotals.tarde} L</p></CardContent></Card>
          <Card><CardContent className="p-4"><p className="text-sm text-gray-600">Total (mês)</p><p className="text-2xl font-bold">{monthTotals.total} L</p></CardContent></Card>
          <Card><CardContent className="p-4"><p className="text-sm text-gray-600">Bruto (mês)</p><p className="text-2xl font-bold text-green-600">{toBRL(monthTotals.bruto)}</p></CardContent></Card>
        </div>
          ...
          <CardContent className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={chartData} margin={{ top: 8, right: 16, bottom: 8, left: 0 }}>
                ...
                {showTotalLine && <Line type="monotone" dataKey="Total" strokeDasharray="4 2" dot />}
              </LineChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
        ...
        <div className="grid gap-4">
                ...
                <CardHeader className="pb-2">
                  <CardTitle className="text-lg flex items-center justify-between">
                    ...
                    <span className="text-sm font-normal text-gray-500">
                      Total: <strong>{d.total} L</strong> • Bruto: <strong className="text-green-600">{toBRL(d.bruto)}</strong>
                    </span>
                  </CardTitle>
                  <CardDescription>
                  ...
                  {d.items.map((it) => (
                    <div key={it.id} className="flex items-center gap-1">
                      <Button variant="outline" size="sm" onClick={() => openEdit(it)}>
                        <Edit className="h-3 w-3 mr-1" /> Editar {it.shift === 'morning' ? 'Manhã' : 'Tarde'}
                      </Button>
                      <Button
                        variant="ghost"
                        size="sm"
          ...
          {tableRows.length === 0 && (
            <Card><CardContent className="py-10 text-center text-gray-500">Sem lançamentos para o período.</CardContent></Card>
          )}
        </div>
      </div>
      ...
      <Dialog open={openForm} onOpenChange={setOpenForm}>
        <DialogContent className="max-w-xl">
          <DialogHeader>
            ...
            <DialogDescription>Um registro por dia/turno. Se existir, será atualizado.</DialogDescription>
          </DialogHeader>
          ...
          <form onSubmit={save}>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                <Label>Litros</Label>
                <Input type="number" step="0.1" min="0" value={form.liters} onChange={(e)=>setForm(f=>({...f, liters:e.target.value}))} required />
              </div>
              <div>
                <Label>Tipo de laticínio</Label>
                <Select value={form.dairy_type} onValueChange={(v)=>setForm(f=>({...f, dairy_type:v, dairy_id:'' , unit_price_at_sale:''}))}>
              ...
              <div className="md:col-span-2">
                <Label>{form.dairy_type === 'official' ? 'Laticínio (oficial)' : 'Meu Laticínio'}</Label>
                <Select value={form.dairy_id} onValueChange={(v)=>setForm(f=>({...f, dairy_id:v, unit_price_at_sale:''}))}>
                  <SelectTrigger><SelectValue placeholder="Selecione um laticínio" /></SelectTrigger>
                  <SelectContent>
                    {(form.dairy_type === 'official' ? officialDairies : myDairies).map(d => (
                      <SelectItem key={d.id} value={d.id}>{d.trade_name || d.name}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
              ...
              <div>
                ...
                <Input type="number" step="0.01" min="0" value={form.unit_price_at_sale}
                       onChange={(e)=>setForm(f=>({...f, unit_price_at_sale:e.target.value}))}
                       placeholder="Ex.: 2.15" />
                <p className="text-xs text-gray-500 mt-1">Preenche sozinho se houver histórico no laticínio.</p>
              </div>

              <div className="md:col-span-2">
                <Label>Observações (opcional)</Label>
                <Input value={form.notes} onChange={(e)=>setForm(f=>({...f, notes:e.target.value}))} placeholder="Ex.: chuva, vacas em cio, etc." />
              </div>
            </div>
