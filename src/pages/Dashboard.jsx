...
const Dashboard = () => {
  ...
  const producerData = useMemo(() => {
    ...
    const dairiesWithPrices = dairies
      .map((dairy) => {
    ...
    const upcomingVaccines = vaccines
      .filter(
        (v) => animalIds.has(v.animal_id) && v.next_due_at && new Date(v.next_due_at) > new Date(),
      )
    ...
    const allProd = JSON.parse(localStorage.getItem("milktech:milk_production") || "[]").filter(
      (x) => x.owner_id === user.id,
    );
  ...
  const helmetTitle =
    user?.role === "producer" ? "Dashboard do Produtor" : "Dashboard da Queijaria";
  const helmetDesc =
    user?.role === "producer"
