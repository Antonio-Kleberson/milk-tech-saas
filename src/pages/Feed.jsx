...
const Feed = () => {
  ...
  const handleRecipeSubmit = (formData) => {
    ...
    if (modalState.editingRecipe) {
      storage.setFeedRecipes(
        allRecipes.map((r) => (r.id === modalState.editingRecipe.id ? { ...r, ...formData } : r)),
      );
  ...
  const handleIngredientSubmit = (formData) => {
    ...
    if (isNaN(value) || value <= 0 || value > 100) {
      toast({
        title: "Erro",
        description: "A proporção deve ser um número entre 0 e 100.",
        variant: "destructive",
      });
      return;
    }
    ...
    if (modalState.editingIngredient) {
      storage.setFeedRecipeItems(
        allItems.map((i) =>
          i.id === modalState.editingIngredient.id ? { ...i, ...newFormData } : i,
        ),
      );
      ...
      storage.setFeedRecipeItems([
        ...
        {
          id: Date.now().toString(),
          recipe_id: modalState.currentRecipeForIngredient.id,
          ...newFormData,
        },
      ]);
  ...
  const deleteRecipe = (recipeId) => {
    ...
    storage.setFeedRecipeItems(
      storage.getFeedRecipeItems().filter((i) => i.recipe_id !== recipeId),
    );
  ...
  return (
    <>
      <Helmet>
        ...
        <meta
          name="description"
          content="Crie e gerencie receitas de ração personalizadas e calcule as quantidades necessárias para sua produção."
        />
        ...
        <div className="grid gap-6">
          {recipes.map((recipe, index) => (
            <RecipeCard
              ...
              onAddIngredient={() =>
                openModal("ingredient", { currentRecipeForIngredient: recipe })
              }
              onEditIngredient={(ingredient) =>
                openModal("ingredient", {
                  editingIngredient: ingredient,
                  currentRecipeForIngredient: recipe,
                })
              }
